...
from src.salinity_engine import compute_tds_profile
...
def execute_sobol_analysis(layers: list, bounds: list, sample_size: int = 2000) -> pd.DataFrame:
    ...
    # Evaluate model rows using our sequential reduction math
    results = []
    for row in param_values:
        # Reuses the exact math pattern: final step of the computed profile
...
def execute_monte_carlo_uncertainty(layers: list, triangular_bounds: dict, iterations: int = 15000) -> np.ndarray:
    ...
    for _ in range(iterations):
        ...
        for layer_name in layers:
            # Draw random residual value from its unique distribution bounds
