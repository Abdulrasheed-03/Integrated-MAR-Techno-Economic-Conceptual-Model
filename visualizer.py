"""
Master Plotting Utilities for Reservoir, Hydrogeological, Statistical, and Financial Suites.

All imports are consolidated at the top of this module. ipywidgets is guarded so that
the module imports cleanly in plain terminal / script mode; the interactive dashboard
function raises an informative error if called outside a Jupyter environment.
"""
import os
import numpy as np
import pandas as pd
...
from .salinity_engine import calculate_adjusted_residual, compute_tds_profile

# ipywidgets only works inside Jupyter; guard the import so the module loads in CLI mode
try:
...
def plot_fluid_volumes(zones: list, thickness: np.ndarray, ooip: np.ndarray, fwip: np.ndarray):
    """Generates a professional stacked bar chart of oil and water volumes per zone."""
    # Normalise bar width proportionally to zone thickness
    ...
    for i in range(len(zones)):
        if fwip[i] > 0:
            ax.text(positions[i], fwip[i] / 2 / 1e6, f"{fwip[i]/1e6:.1f}",
                    ha="center", va="center", color="black", fontsize=10)
        if ooip[i] > 0:
            ax.text(positions[i], (fwip[i] + ooip[i] / 2) / 1e6, f"{ooip[i]/1e6:.1f}",
    ...
    plt.tight_layout()
    out_file = os.path.join(OUTPUT_DIR, "Module1_Volumetrics.png")
    plt.savefig(out_file, dpi=300, bbox_inches="tight")
    print(f"[SUCCESS] Figure saved to {out_file}")
    plt.show()


# ---------------------------------------------------------------------------
# Module 2 — 4D Migration Animation
# ---------------------------------------------------------------------------
...
def generate_4d_animation(time_years: np.ndarray, zones_order: list, history_arr: dict, n_steps: int):
    ...
    sw_init  = np.array([1.0, 1.0, 1.0, 0.51, 0.23])
    sw_final = np.array([0.9, 0.9, 0.9, 0.95, 0.98])
    ...
    for t in range(0, n_steps, 2):
        frame_data = [
            ...
            go.Scatter3d(
                x=time_years[:t+1], y=[3] * len(time_years[:t+1]), z=cum_influx_4[:t+1],
                mode="lines", line=dict(color="red", width=4), name="Zone4 Influx",
            ),
            go.Scatter3d(
                x=time_years[:t+1], y=[4] * len(time_years[:t+1]), z=cum_influx_5[:t+1],
                mode="lines", line=dict(color="orange", width=4), name="Zone5 Influx",
            ),
            go.Scatter3d(
                x=time_years[:t+1], y=[-1] * len(time_years[:t+1]), z=pressure_norm[:t+1],
                mode="lines", line=dict(color="cyan", width=3, dash="dash"), name="Pressure Decline",
            ),
        ]
    ...
    out_file = os.path.join(OUTPUT_DIR, "Module2_Reservoir_4D_Animation.html")
    pio.write_html(fig, file=out_file, auto_open=False)
    print(f"[SUCCESS] 4D Interactive animation file saved as '{out_file}'.")


# ---------------------------------------------------------------------------
# Module 3 — Salinity / TDS Profile
# ---------------------------------------------------------------------------

def render_static_salinity_plot(layers: list, initial_tds: float, tds_base: list, tds_amend: list):
    """Generates the primary performance chart mapping TDS decline against depth.

    Parameters:
        layers (list): Layer dictionaries containing 'name' and 'thickness_m'.
        initial_tds (float): Starting Total Dissolved Solids concentration (mg/L).
        tds_base (list): TDS concentration at each profile boundary — baseline case (mg/L).
        tds_amend (list): TDS concentration at each profile boundary — amended case (mg/L).
    """
    depths = [0] + list(np.cumsum([l["thickness_m"] for l in layers]))

    plt.figure(figsize=(10, 5), dpi=300)
    plt.plot(depths, tds_base,  marker="o", color="#1f77b4", linewidth=2, label="Baseline TDS")
    plt.plot(depths, tds_amend, marker="s", color="#2ca02c", linewidth=2, label="With Engineered Amendments")
    ...
    plt.tight_layout()
    out_file = os.path.join(OUTPUT_DIR, "Module3_Salinity_Profile.png")
    plt.savefig(out_file, dpi=300, bbox_inches="tight")
    print(f"[SUCCESS] Figure saved to {out_file}")
    plt.show()
...
def run_interactive_salinity_dashboard(layers: list, initial_tds: float, amendments_baseline: list):
    """Constructs the interactive slider control layout for Jupyter Notebook environments.

    Raises:
        RuntimeError: If called outside a Jupyter environment where ipywidgets is unavailable.
    """
    if not _IPYWIDGETS_AVAILABLE:
        raise RuntimeError(
            "ipywidgets is not available in this environment. "
            "run_interactive_salinity_dashboard() requires a Jupyter Notebook or JupyterLab session."
        )
    ...
    # Build dynamic slider controls
    sliders = {}
    for i, l in enumerate(layers):
        sliders[f"{l['name']}_residual"] = FloatSlider(
            value=l["residual"], min=0.05, max=1.0, step=0.05, description=f"{l['name'][:5]} Res"
        )
        sliders[f"{l['name']}_biochar"] = FloatSlider(
            value=amendments_baseline[i]["biochar_vv"], min=0, max=0.2, step=0.01, description="↳ Biochar"
        )
        sliders[f"{l['name']}_zeolite"] = FloatSlider(
            value=amendments_baseline[i]["zeolite_vv"], min=0, max=0.2, step=0.01, description="↳ Zeolite"
        )
        sliders[f"{l['name']}_ac"] = FloatSlider(
            value=amendments_baseline[i]["ac_vv"], min=0, max=0.2, step=0.01, description="↳ Act Carbon"
        )
...
# ---------------------------------------------------------------------------
# Module 4 — Statistical Publication Panel
# ---------------------------------------------------------------------------
...
def plot_statistical_publication_panel(sobol_df: pd.DataFrame, profiles_matrix: np.ndarray, depths: np.ndarray):
    ...
    ax_a.set_xlabel(r"Total-order Sensitivity Index ($S_T$)", fontsize=11)
    ...
    plt.tight_layout()
    out_file = os.path.join(OUTPUT_DIR, "Module4_Statistical_Panel.png")
    plt.savefig(out_file, dpi=300, bbox_inches="tight")
    print(f"[SUCCESS] Figure saved to {out_file}")
    plt.show()
...
# ---------------------------------------------------------------------------
# Module 5 — Financial Valuation Panel
# ---------------------------------------------------------------------------
...
def plot_financial_valuation_panel(financial_arr: np.ndarray, extended_arr: np.ndarray, tornado_data: tuple):
    ...
    # Left Subplot: NPV Distributions
    ax_a = axes[0]
    sns.kdeplot(financial_arr, label=f"Financial Framework (P50={fin_percentiles[1]:.2f})",
                linewidth=2, color="#1f77b4", ax=ax_a)
    sns.kdeplot(extended_arr,  label=f"Extended Framework (P50={ext_percentiles[1]:.2f})",
                linewidth=2, color="#ff7f0e", ax=ax_a)
    ...
    stat_box_text = (
        f"Risk Threshold P(NPV < 0):\n"
        f"Pure Financial: {loss_probability_financial:.1%}\n"
        f"ESG Extended:   {loss_probability_extended:.1%}"
    )
    ax_a.text(0.03, 0.95, stat_box_text, transform=ax_a.transAxes, verticalalignment="top",
    ...
    ax_b.invert_yaxis()
    ax_b.set_title("Local Sensitivity Matrix (Tornado Analysis Impact Deviation)",
    ...
    plt.tight_layout()
    out_file = os.path.join(OUTPUT_DIR, "Module5_NPV_SideBySide.png")
